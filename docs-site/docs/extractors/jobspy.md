...
- In Map radius mode, JobOps resolves the nearest named place in the selected circle and passes it with JobSpy's native distance option. This avoids running once for every expanded settlement while the central location matcher still enforces the shared radius intent.
